    ...
    def add_encoder(self):
        ...
        with tf.variable_scope("sentence_src"):
            if self.config.net_sentence == 'last':
                ...
                self.mask1 = tf.sequence_mask(self.len_src, dtype=tf.float32)
#                self.mask2 = tf.cast(self.mask1, tf.float32) #tf.to_float(self.mask1) 
                mask = tf.expand_dims(self.mask2, 2) #[B,Ss] => [B,Ss,1]
...
#            print("ref_batch {}".format(ref_batch))
            ...
            embed_snt_src_batch, mask1 = self.sess.run([self.embed_snt, self.mask1], feed_dict=fd)
